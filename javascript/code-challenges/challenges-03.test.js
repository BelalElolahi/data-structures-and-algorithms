...
const typeNum = (arr) => {
    ...
    let filterNumber = arr.filter(item => {
...
const containsAnd = (arr) => {
    // Solution code here...
    let filterStringAnd = arr.filter(item => {
...
const oddValues = (arr) => {
    ...
    let oddNumberArray = arr.filter(number => {
        if (number % 2 === 1) {
            return number;
...
const evenOddNumericValues = (arr) => {
    ...
    let oodEvenNum = arry.map((number) => {
        if (number % 2 === 0) {
            oddNumberArray.push('even');
        } else {
            oddNumberArray.push('odd');
        }
